...
export function buildPilotPlannerContext(
  ...
  const systemPrompt = [
    ...
    "1. UC1 (Needs Input): If <checklist_summary> status is 'needs_input', unconfirmed_business is true, missing_fields is non-empty, or an assignment lacks an exact verified Trello member ID from trusted source/context, return PlannerResult kind 'clarification' with a non-empty question. Do not emit a plan or steps. Never guess an ambiguous assignee.",
    "Assignment requires an exact verified Trello member ID from trusted source/context. A username, display name, or unverified ID is insufficient. If assignment is requested without that verification, return PlannerResult kind 'clarification' with a non-empty question; do not emit a plan, steps, or write.",
    ...
    'In each tool reference, put the server and local tool name in separate fields: for example server: "trello", name: "create_card". Do not put the qualified name "trello.create_card" in the name field.',
    ...
    'Every write step must include a non-empty DSL idempotency_key; for the create-card step use idempotency_key: "${runtime.run_id}_create_card" as a step field, not a tool argument. The evaluator does not add this DSL field; the planner must supply it in the proposed plan before approval.',
    ...
    "Input declarations must not be both required and defaulted: required true implies default_present false and default_value null. If default_present is true, required must be false. A default must be a string, number or boolean matching the declared input type, encoded with the matching scalar wire kind; never use array, object or null defaults.",
    'Workflow outputs must be DSL references such as "${steps.read_request.output.row}". Never write prose, a status sentence, "result", or a bare step reference into workflow outputs. For google_sheets.read_request, valid output paths include output.row, output.checklist, output.sourceKey, and output.sourceRevision. For trello.get_card, valid output paths include output.id, output.name, output.desc, output.idList, output.due, output.idMembers, and output.url. If no output is needed, leave outputs empty.',
  ...
  const rawUserPrompt = [
    ...
    ...(trustedTargetsPrompt ? [trustedTargetsPrompt,
      ...
      "Use only the planning args shown above for tool arguments; include the required DSL idempotency_key on the write step as instructed in the planning rules.",
    ] : []),
    "",
    "=== INTAKE EVIDENCE ===",
