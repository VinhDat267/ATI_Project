...
> **Trạng thái 26/09/2026:** owner isolation và UC1/UC3 đã qua API, PostgreSQL và Chromium với Sheets/Trello giả; UC2 approval đã qua API/DB/browser với Trello giả. Một ca sandbox đã đọc Google Sheets/Trello thật, tạo đúng một card Trello sau approval, lưu receipt PostgreSQL và xác minh card bằng GET; chủ project đã xác nhận nội dung card. Sau khi diagnostic xác định lỗi input key và required/default, freeze `d35a9af` của Gemini 3.1 Flash-Lite đạt **26/26 structural PASS trên public**, không lỗi provider hoặc unsafe proposal trong lần chạy này. Review nội dung public còn findings; 20 holdout chưa chạy và chất lượng AI chưa nghiệm thu. `PILOT_V2_WRITE_ENABLED` tắt mặc định; pilot vẫn `HANDOFF_BLOCKED`. Xem [baseline](docs/BASELINE.md) và [runbook](docs/PILOT-V2-RUNBOOK.md) trước khi diễn giải kết quả test.
...
| AI quality v2 | [Public mới](docs/ai-evidence/PILOT-V2-AI/PUBLIC-2026-09-26.md) trên `d35a9af` có 26/26 structural PASS, 0 provider failures; [scope model-only](docs/ai-evidence/PILOT-V2-AI/MODEL-SCOPE-2026-09-26.md) dùng 26 public + 20 holdout mới | `PARTIAL_NOT_MEASURED`: review nội dung public còn findings; holdout chưa chạy, chưa hoàn tất semantic adjudication/semantic-QE comparison |
