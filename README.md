...
> **Trạng thái 26/09/2026:** owner isolation và UC1/UC3 đã qua API, PostgreSQL và Chromium với Sheets/Trello giả; UC2 approval đã qua API/DB/browser với Trello giả. Một ca sandbox đã đọc Google Sheets/Trello thật, tạo đúng một card Trello sau approval, lưu receipt PostgreSQL và xác minh card bằng GET; chủ project đã xác nhận nội dung card. Freeze mới `2011169` của Gemini 3.1 Flash-Lite đạt **26/26 strict public PASS** (26 provider successes, usage được báo cáo, 0 USD cap). Holdout đã mở sau khi public qua gate nhưng execution dừng trước attempt đầu tiên vì cả 20 fixture thiếu `tabId`; holdout là `NOT_RUN`, không được sửa và tái sử dụng. Semantic adjudication, user acceptance và SaaS handoff còn mở; `PILOT_V2_WRITE_ENABLED` tắt mặc định. Xem [bằng chứng campaign mới](docs/ai-evidence/PILOT-V2-AI/PROVIDER-PUBLIC-FOLLOWUP-2026-09-26.md), [baseline](docs/BASELINE.md) và [runbook](docs/PILOT-V2-RUNBOOK.md).
...
| AI quality v2 | [Campaign mới](docs/ai-evidence/PILOT-V2-AI/PROVIDER-PUBLIC-FOLLOWUP-2026-09-26.md): 26/26 strict public PASS, 0 provider failures, 0 USD cap | `PARTIAL_NOT_MEASURED`: holdout execution `NOT_RUN` vì 20 fixture thiếu `tabId`; semantic adjudication, semantic/QE comparison và user acceptance còn mở |
