...
- G1 tổng thể vẫn **PARTIAL** (`TECHNICAL_PASS_OVERALL_PARTIAL`): backend API-GATE đã đạt `API_TECHNICAL_PASS`; WEB-03 đã đạt `WEB_BROWSER_EXERCISED_PASS` với fixture browser, live API/DB browser E2E, polling và bundle/cleanup gate. Rubric chính thức và công việc nhóm đại diện còn `OPEN`; application-level provider-backed LLM E2E/quality evaluation vẫn `NOT_RUN`. Visual direction/Design System vẫn là quyết định mở của người dùng. Xem [system design](docs/superpowers/specs/2026-09-15-platform-system-design.md) trước khi mở rộng frontend và [web status](docs/WEB-STATUS.md).
