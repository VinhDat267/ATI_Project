...
> **Trạng thái 26/09/2026:** owner isolation và UC1/UC3 đã qua API, PostgreSQL và Chromium với Sheets/Trello giả; UC2 approval đã qua API/DB/browser với Trello giả. Một ca sandbox đã đọc Google Sheets/Trello thật, tạo đúng một card Trello sau approval, lưu receipt PostgreSQL và xác minh card bằng GET; chủ project đã xác nhận nội dung card. Gemini 3.7 từng trả HTTP 503, Gemini 3.8 từng trả HTTP 429. Chiến dịch Gemini 3.1 Flash-Lite ở freeze `c9df557` đã chạy 26/26 ca public: 25 đạt chấm cấu trúc, một ca đề xuất write sai khi người nhận mơ hồ. Chiến dịch mới trên mã guard dừng ở lời gọi thứ 9 vì đầu ra plan không hợp lệ theo DSL; holdout chưa chạy và chất lượng AI chưa nghiệm thu. `PILOT_V2_WRITE_ENABLED` tắt mặc định; pilot vẫn `HANDOFF_BLOCKED`. Xem [baseline](docs/BASELINE.md) và [runbook](docs/PILOT-V2-RUNBOOK.md) trước khi diễn giải kết quả test.
...
| AI quality v2 | [Freeze cũ](docs/ai-evidence/PILOT-V2-AI/PROBE-2026-09-26.md) Gemini 3.1 Flash-Lite có 26/26 observation, 25/26 structural PASS; [scope model-only](docs/ai-evidence/PILOT-V2-AI/MODEL-SCOPE-2026-09-26.md) dùng 26 public + 20 holdout mới | `PARTIAL_NOT_MEASURED`: freeze cũ có V2-10-en đề xuất write sai; freeze mới dừng ở V2-03-vi do DSL plan không hợp lệ sau 9 calls; holdout chưa chạy, chưa adjudicate nội dung độc lập |
