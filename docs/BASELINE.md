...
CODE_TESTED áp dụng cho thư viện DSL. DB 5 migrations, 8 public `task_hub` tools, 2 public `filesystem` tools và controller/engine hai server đã có kiểm chứng PostgreSQL/MCP thật. FS-05 đạt **TECHNICAL PASS** cho E01–E14; fresh FS-06 gate đạt **258 passed, 1 skipped**. Luồng CLI tạo một preview/approval, write tuần tự, trace, cancel và recovery không resume đã triển khai. `SPEC_ONLY`/`NOT_RUN` còn áp dụng cho HTTP/session/UI/polling, LLM/retrieval/replan và BullMQ. G1 tổng thể vẫn **PARTIAL** (`TECHNICAL_PASS_OVERALL_PARTIAL`): rubric chính thức và công việc nhóm đại diện còn `OPEN`; chưa có HTTP/browser E2E, polling hoặc kết quả AI. Xem [filesystem status](G1-FILESYSTEM-STATUS-2026-09-13.md) và [rubric map](G1-RUBRIC-MAP.md).
