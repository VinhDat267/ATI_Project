...
| N03 | `.env.example` đặt `LLM_MODEL=claude-sonnet-4-6` — không khớp định danh model Anthropic hiện hành nào. File đã tự ghi chú "candidates, not verified provider probes" nên không phải lỗi, nhưng cần chốt bằng model id thật trước khi đưa vào `experiment-manifest.json`/FR-CON-08. | Nhắc việc | Xác nhận model id thật (qua provider docs) ở tuần 1/4 trước khi pin vào manifest; không tự sửa trong audit này vì cần xác nhận nguồn. |
...
N03 (`LLM_MODEL` placeholder trong `.env.example`) và N04 (chưa init Git) vẫn để mở — cần quyết định của nhóm, không tự sửa.
