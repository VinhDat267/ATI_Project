...
Từ root repo, mở **hai PowerShell riêng** sau khi Docker/PostgreSQL sandbox đã chạy và mã đã build. Terminal 1 chạy `./scripts/pilot-owner-acceptance.ps1 -Mode api`; script tạo mật khẩu một lần, chỉ hiện tại terminal đó, ép Trello write và AI provider calls tắt. Terminal 2 chạy `./scripts/pilot-owner-acceptance.ps1 -Mode web`; mở URL script hiển thị rồi đăng nhập bằng email/mật khẩu ở terminal 1. Đóng hai terminal để dừng dịch vụ. Script chưa được chạy end-to-end trong phiên Codex này; nếu API hoặc web báo lỗi, ghi `BLOCKED` và gửi mã lỗi đã che credential.
