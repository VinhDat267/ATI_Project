...
| Nguồn gốc lần chạy tạo lại | Chưa có | Đề xuất: POST /runs nhận `origin: { run_id, reason }` (`reason`: `reconcile_seen`, `reconcile_not_seen`, `reconcile_confirmed`, `expired`, `failed`, `needs_input`); máy chủ kiểm tra run gốc thuộc cùng người dùng; GET /runs/:id trả `origin` và ghi vào chứng cứ |
