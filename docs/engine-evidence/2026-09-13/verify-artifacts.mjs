...
const sourceFiles = ['package.json','package-lock.json','tsconfig.json','scripts/check-engine.mjs','scripts/g1-status.mjs'];
for (const directory of ['packages/engine/src','packages/engine/tests','packages/engine/dist','packages/db/src','apps/mcp-task-hub/src']) {
  for (const file of fs.readdirSync(path.join(root,directory))) {
    if (/\.(?:ts|mjs|js)$/.test(file)) sourceFiles.push(`${directory}/${file}`);
  }
}
const result = { checked_at:new Date().toISOString(), node:process.version,
  ...
  source_sha256:Object.fromEntries(sourceFiles.map(p=>[p,createHash('sha256').update(fs.readFileSync(path.join(root,p))).digest('hex')])),
  limitations:['JSON Schema structural checks do not replace Zod refinements or runtime authorization','Static artifacts only; PostgreSQL/MCP/controller/CLI checks are recorded separately in check-engine.log and controller-observations.json; HTTP/UI/LLM/BullMQ remain NOT_RUN'], failures };
