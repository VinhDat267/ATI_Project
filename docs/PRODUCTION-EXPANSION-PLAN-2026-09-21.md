...
- OIDC BFF đã có implementation kỹ thuật có feature-gate: migration `0009_oidc_identity.sql`, hash-only identity/session/transaction repository, provider-neutral discovery/PKCE/JWKS validation, `/auth/oidc/start|callback`, `/auth/me`, CSRF/SameSite/origin checks, web cookie transport/proxy và UUID-scoped engine/gateway factory. Local technical E2E đã PASS qua discovery, callback/PKCE/JWKS, durable session, revoke và hai-user owner matrix; manifest là `docs/auth-evidence/OIDC-01/local-20260921071204886-9465dad2-9559-473a-9487-35fb9e9ed23c/manifest.json`. Runtime vẫn tắt mặc định; issuer/tenant deployment, per-principal AI campaign wiring và two-user live acceptance với provider thật còn `OPEN`.
