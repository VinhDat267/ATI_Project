...
- Code checkout dùng cho backend gate kết thúc ở `0b3ff41`; commit tài liệu này ghi nhận trạng thái đó và worktree hiện sạch.
- Full integrated gate đã pass trên commit `a0a0ae4`; backend gate trên code commit `0b3ff41` cũng pass: DSL 43, engine unit 341 (1 skipped), API unit 57, web unit 128; MCP integration 64, engine integration 102, API integration 40. WEB-03 pass đủ 6 browser/build gates trên code commit `20dc8bd`.
...
- API có `POST /auth/logout` để revoke bearer session hiện tại; cookie/OIDC logout hiện cũng revoke durable opaque sessions khi OIDC được bật.
- OIDC BFF đã có implementation kỹ thuật có feature-gate: migration `0009_oidc_identity.sql`, hash-only identity/session/transaction repository, provider-neutral discovery/PKCE/JWKS validation, `/auth/oidc/start|callback`, `/auth/me`, CSRF/SameSite/origin checks, web cookie transport/proxy và UUID-scoped engine/gateway factory. Local technical E2E đã PASS qua discovery, callback/PKCE/JWKS, durable session, restart/revoke và hai-user owner matrix; manifest là `docs/auth-evidence/OIDC-01/local-20260921071503494-6321629d-e85e-4efa-9e37-e057ad822183/manifest.json`. Runtime vẫn tắt mặc định; issuer/tenant deployment, per-principal AI campaign wiring và two-user live acceptance với provider thật còn `OPEN`.
- OIDC-01 release evidence runner đã được nối vào `npm run check:oidc`; manifest kỹ thuật hiện tại ghi `technical=PASS`, `overall=OPEN` vì chưa có provider/tenant được cấp quyền và live acceptance. Manifest không chứa secret/token và được lưu dưới `docs/auth-evidence/OIDC-01/`.
- API hiện tách `SessionAuthority` injectable khỏi `SessionStore`; memory implementation vẫn là mặc định khi OIDC tắt, còn durable implementation được chọn ở main khi OIDC bật.
